	...
	rootCmd := &cobra.Command{
		...
		Run: func(cmd *cobra.Command, args []string) {
			...
			if oidcAllowedGroups != "" {
				oidcAllowedGroupsList = strings.Split(oidcAllowedGroups, ",")
				for i := range oidcAllowedGroupsList {
					oidcAllowedGroupsList[i] = strings.TrimSpace(oidcAllowedGroupsList[i])
				}
			}
